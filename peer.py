...
class PeerChatRoom(threading.Thread):
    ...
    def run(self):
        ...
        if message[0] == "JOIN-ROOM-SUCCESS":
            ...
            send_thread = threading.Thread(target=self.write, args=(self.username, self.chatroomname,self.roomport))
            send_thread.start()
            receive_thread.join()
            send_thread.join()
    ...
    def write(self, peername, room_name=None ,roomport=None):
        time.sleep(1)
        while True:
            try:
                content = input("")
                if content == "QUIT":
                    ...
                    if response.startswith("LEAVE-ROOM-SUCCESS"):
                        message = f"REMOVE-PORT-ROOM {peername} {room_name} {roomport}"
                        self.tcpClientServerSocket.send(message.encode())
                        # response = self.tcpClientServerSocket.recv(1024).decode()
                        self.roomflag = None
                        self.joinedroomflag = None
