        ...
        while choice != "8":
         ...
         ##############################ADDED##################################
                ...
                if room_status:
                    ...
                    self.peerClient = PeerClient(self.registryPort, self.peerServerPort , self.loginCredentials[0], self.peerServer, None, 1,self.room_name,self.registryName,room_Port)
                    self.peerClient.start()
                    self.peerClient.join()
                    # self.tcpClientSocket = socket(AF_INET, SOCK_STREAM)
                    #
            ...
            # elif  choice == ":quit":
            #
            #     self.leave_chat_room(self.loginCredentials[0],self.room_name)
...
#################added#####################################
    ...
    # def leave_chat_room(self ,Peername,chatroom_name):
    #     message = "LEAVE-ROOM "+ Peername+chatroom_name
    #     self.tcpClientSocket.send(message.encode())
    # #####################################ADDED#######################
