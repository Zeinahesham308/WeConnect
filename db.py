...
class DB:
    def __init__(self):
        ...
        # Create the "ports" collection if it doesn't exist
    ...
    def add_randomed_port(self, portno):
        port_data = {
            "port": portno
        }
        self.db.ports.insert_one(port_data)

    def get_randomed_ports(self):
        # Retrieve all ports from the "ports" collection
        ports_cursor = self.db.ports.find({}, {"port": 1, "_id": 0})
        return [port["port"] for port in ports_cursor]



    def delete_port(self, portno):
        # Delete the specified port from the "ports" collection
        self.db.ports.delete_one({"port": portno})
