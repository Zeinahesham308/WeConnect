...
class ClientThread(threading.Thread):
    ...
    def generate_random_port(self):
        # Set the default range for dynamic or private ports
        min_port = 49152
        max_port = 65535
        # Get existing ports from the database
        existing_ports = db.get_randomed_ports()
        while True:
            random_port = random.randint(min_port, max_port)

            # Check if the port is not in the exclusion list, not reserved, and not in existing ports
            if random_port  not in existing_ports and random_port not in [15600, 15500]:
                db.add_randomed_port(random_port)
                return random_port
